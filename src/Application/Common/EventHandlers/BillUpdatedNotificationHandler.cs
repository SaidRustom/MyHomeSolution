...
public sealed class BillUpdatedNotificationHandler(
    IApplicationDbContext dbContext,
    IRealtimeNotificationService realtimeService,
    IDateTimeProvider dateTimeProvider)
    : INotificationHandler<BillUpdatedEvent>
{
    public async Task Handle(BillUpdatedEvent notification, CancellationToken cancellationToken)
    {
        ...
        var recipientUserIds = bill.Splits
            .Where(s => s.UserId != notification.UpdatedByUserId)
            .Select(s => s.UserId)
            .Distinct();
        ...
        foreach (var recipientUserId in recipientUserIds)
        {
            ...
            {
                Title = $"Bill updated: {bill.Title}",
                Description = $"The bill '{bill.Title}' has been updated.",
